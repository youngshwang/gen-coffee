...
module.exports = (grunt) ->
  ...
  grunt.initConfig
    ...
    watch:<% if(filters.useclient) { %>
      ...
      stylus:
        files: ['<%%= yeoman.client %>/{app,components}/**/*.styl']
        tasks: ['stylus', 'autoprefixer']<% } %><% if(filters.sass) { %>
      ...
      sass:
        files: ['<%%= yeoman.client %>/{app,components}/**/*.{scss,sass}']
        tasks: ['sass', 'autoprefixer']<% } %><% if(filters.less) { %>
      ...
      less:
        files: ['<%%= yeoman.client %>/{app,components}/**/*.less']
        tasks: ['less', 'autoprefixer']<% } %><% if(filters.jade) { %>
    ...
    nodemon:
      debug:<% if(filters.coffee) { %>
        script: '.server/app.js'<% } else {%>
        script: 'server/app.js'<% } %>
        options:
          nodeArgs: ['--debug-brk']
          env:
    ...
    env:
      ...
      all: localConfig<% if(filters.jade) { %>
    ...
    sass:
      server:
        ...
        files:
          '.tmp/app/app.css' : '<%%= yeoman.client %>/app/app.scss'<% } %><% if(filters.less) { %>
